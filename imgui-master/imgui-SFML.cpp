...
bool imageAnimButtonImpl(const sf::Texture& texture, const sf::Texture& texture2, const sf::FloatRect& textureRect, const sf::FloatRect& textureRect2,
                         const sf::Vector2f& size, const int framePadding, const sf::Color& bgColor, const sf::Color& tintColor);
...
bool ImageAnimButton(const sf::Texture& texture, const sf::Texture& texture2,
    const int framePadding, const sf::Color& bgColor, const sf::Color& tintColor)
{
    return ImageAnimButton(texture, texture2, static_cast<sf::Vector2f>(texture.getSize()), framePadding, bgColor, tintColor);
}

bool ImageAnimButton(const sf::Texture& texture, const sf::Texture& texture2, const sf::Vector2f& size,
    const int framePadding, const sf::Color& bgColor, const sf::Color& tintColor)
{
    sf::Vector2f textureSize = static_cast<sf::Vector2f>(texture.getSize());
    sf::Vector2f texture2Size = static_cast<sf::Vector2f>(texture2.getSize());
    return ::imageAnimButtonImpl(texture, texture2, sf::FloatRect(0.f, 0.f, textureSize.x, textureSize.y), sf::FloatRect(0.f, 0.f, texture2Size.x, texture2Size.y), size, framePadding, bgColor, tintColor);
}

bool ImageAnimButton(const sf::Sprite& sprite, const sf::Sprite& sprite2,
    const int framePadding, const sf::Color& bgColor, const sf::Color& tintColor)
{
    sf::FloatRect spriteSize = sprite.getGlobalBounds();
    return ImageAnimButton(sprite, sprite2, sf::Vector2f(spriteSize.width, spriteSize.height), framePadding, bgColor, tintColor);
}

bool ImageAnimButton(const sf::Sprite& sprite, const sf::Sprite& sprite2, const sf::Vector2f& size,
    const int framePadding, const sf::Color& bgColor, const sf::Color& tintColor)
{
    const sf::Texture* texturePtr = sprite.getTexture();
    const sf::Texture* texturePtr2 = sprite2.getTexture();
    if (!texturePtr || !texturePtr2) { return false; }
    return ::imageAnimButtonImpl(*texturePtr, *texturePtr2, static_cast<sf::FloatRect>(sprite.getTextureRect()), static_cast<sf::FloatRect>(sprite2.getTextureRect()), size, framePadding, bgColor, tintColor);
}
...
bool imageAnimButtonImpl(const sf::Texture& texture, const sf::Texture& texture2, const sf::FloatRect& textureRect,
                         const sf::FloatRect& textureRect2, const sf::Vector2f& size, const int framePadding,
                         const sf::Color& bgColor, const sf::Color& tintColor)
{
    sf::Vector2f textureSize = static_cast<sf::Vector2f>(texture.getSize());
    ...
    ImVec2 uv2(textureRect2.left / textureSize.x, textureRect2.top / textureSize.y);
    ImVec2 uv3((textureRect2.left + textureRect2.width)  / textureSize.x,
               (textureRect2.top  + textureRect2.height) / textureSize.y);

    return ImGui::ImageAnimButton((void*)texture.getNativeHandle(), (void*)texture2.getNativeHandle(), size, uv0, uv1, uv2, uv3, framePadding, bgColor, tintColor);
}
